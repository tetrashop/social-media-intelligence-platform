...
async function initModel() {
  try {
    ...
    console.error('خطا در بارگذاری مدل، استفاده از fallback:', e);
    nn = null;
  }
}
...
async function trainDefaultExamples() {
  ...
  console.log('✓ آموزش اولیه انجام و ذخیره شد');
}
...
async function processMessage(message, sessionId) {
  if (!modelReady) await new Promise(resolve => setTimeout(resolve, 50));
