...
router.post('/message', async (req, res) => {
  try {
    ...
    if (!message || !sessionId) return res.status(400).json({ response: 'پیام و نشست الزامی است.' });
...
router.post('/train', async (req, res) => {
  try {
    ...
    if (!dataset || !Array.isArray(dataset)) return res.status(400).json({ error: 'آرایه‌ای از {text, intent} ارسال کنید.' });
